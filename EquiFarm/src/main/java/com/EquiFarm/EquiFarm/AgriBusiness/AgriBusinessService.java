...
public class AgriBusinessService {
    ...
    public ApiResponse<?> getAllAgribusinessInValueChain(Long valueChainId) {
        try {
            ...
            List<FarmersResponse> farmersResponses = agriBusinessList
                    .stream()
                    .map(agribusiness -> modelMapper
                            .map(agribusiness, FarmersResponse.class))
                    .toList();
            return new ApiResponse<>("Agribusiness fetched successfully", farmersResponses, HttpStatus.OK.value());

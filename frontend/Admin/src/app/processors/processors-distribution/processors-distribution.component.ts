...
@Component({
  selector: 'app-processors-distribution',
  templateUrl: './processors-distribution.component.html',
  styleUrls: ['./processors-distribution.component.css']
})
export class ProcessorsDistributionComponent implements OnInit {

  @ViewChild("chart") chart: ChartComponent;
  public chartOptions: Partial<ChartOptions>;
  // locations: any;
  processors: any[] = [];
  apiKey = 'ge-7752d136385e0e54';
    ...
    this.processorsService.getAllProcessors().subscribe({
      next: ((res) => {
        if (res.statusCode === 200) {
          ...
          const coordinates = this.processors.map(processor => ({
            latitude: processor.user.latitude,
            longitude: processor.user.longitude,
          }));
  ...
  private getLocationAddress(params: HttpParams): Observable<any> {
    return this.processorsService.getLocationDetails(params)
  }

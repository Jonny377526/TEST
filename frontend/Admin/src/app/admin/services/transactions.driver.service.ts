...
export class TransactionsDriverService {
  ...
  getTransactionUserIds(): Observable<string[]> {
    return this.http.get<any[]>(`${environment.baseUrl}/api/v1/transactions/fetch/all`).pipe(
      map(transactions => transactions.map(transaction => transaction.sellerUserId))
    );
  }
  getUserDetails(id: string): Observable<any> {
    return this.http.get<any>(`${environment.baseUrl}/api/v1/user/get/all/${id}`);
  }

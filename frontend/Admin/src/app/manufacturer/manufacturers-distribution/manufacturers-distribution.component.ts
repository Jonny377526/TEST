...
@Component({
  selector: 'app-manufacturers-distribution',
  templateUrl: './manufacturers-distribution.component.html',
  styleUrls: ['./manufacturers-distribution.component.css']
})
export class ManufacturersDistributionComponent implements OnInit {
    ...
    this.manufacturerService.getAllManufacturers().subscribe({
      next: ((res) => {
        if (res.statusCode === 200) {
          ...
          const coordinates = this.manufacturers.map(manufacturer => ({
            latitude: manufacturer.user.latitude,
            longitude: manufacturer.user.longitude,
          }));
  ...
  private getLocationAddress(params: HttpParams): Observable<any> {
    return this.manufacturerService.getLocationDetails(params)
  }
